...
func TestUseCaseTreeAPI(t *testing.T) {
	...
	for _, n := range []struct{ ja, en string }{
		...
		if s != 200 {
			t.Fatalf("upsert %s: %d %s", n.en, s, raw)
		}
	}
	...
	if s != 200 {
		t.Fatalf("list level=top: %d %s", s, raw)
	}
	var listed struct {
		Total    int `json:"total"`
		UseCases []struct {
	...
	json.Unmarshal(raw, &listed)
	if listed.Total != 3 {
		t.Fatalf("top initial total: %d", listed.Total)
	}
	for _, r := range listed.UseCases {
		if r.ParentID != "" {
			t.Errorf("expected empty parent_id, got %q", r.ParentID)
		}
		if r.ChildCount != 0 {
			t.Errorf("expected ChildCount=0, got %d", r.ChildCount)
		}
	}
	...
	if s != 200 {
		t.Fatalf("create meta: %d %s", s, raw)
	}
	var metaRes struct {
		ID string `json:"id"`
	}
	...
	for _, leaf := range []struct{ slug, en string }{
		...
		if s != 200 {
			t.Fatalf("repoint %s: %d %s", leaf.slug, s, raw)
		}
	}
	...
	json.Unmarshal(raw, &listed)
	if listed.Total != 2 {
		t.Fatalf("after stack: top total %d", listed.Total)
	}
	gotMetaWithChildren := false
	for _, r := range listed.UseCases {
		if r.ID == metaRes.ID && r.ChildCount == 2 {
			gotMetaWithChildren = true
		}
	}
	if !gotMetaWithChildren {
		t.Errorf("meta should be top-level with ChildCount=2: %+v", listed.UseCases)
	}

	// GET meta — includes children[].
	s, raw = doJSON(t, "GET", base+"/v1/use_cases/"+metaRes.ID, tok, nil, nil)
	if s != 200 {
		t.Fatalf("get meta: %d %s", s, raw)
	}
	var detail struct {
		UseCase  map[string]any   `json:"use_case"`
	...
	if detail.Parent != nil {
		t.Errorf("meta parent should be nil, got %v", detail.Parent)
	}
	if len(detail.Children) != 2 {
		t.Errorf("meta children: want 2, got %d", len(detail.Children))
	}
	...
	json.Unmarshal(raw, &listed)
	if listed.Total != 2 {
		t.Fatalf("drilldown count: want 2, got %d", listed.Total)
	}
	...
	if s != 200 {
		t.Fatalf("un-root: %d %s", s, raw)
	}
	s, raw = doJSON(t, "GET", base+"/v1/use_cases?level=top", tok, nil, nil)
	json.Unmarshal(raw, &listed)
	// meta + leafC + un-rooted leafA = 3 at top level now.
	if listed.Total != 3 {
		t.Fatalf("after un-root: top total %d, want 3", listed.Total)
	}
}
...
func TestUseCaseLimitClamps(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		if s != 200 {
			t.Fatalf("upsert leaf %d: %d %s", i, s, raw)
		}
	}
	...
	json.Unmarshal(raw, &got)
	if got.Total != 5 {
		t.Fatalf("total: want 5, got %d", got.Total)
	}
